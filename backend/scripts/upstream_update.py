...
def command_commands(args: argparse.Namespace) -> None:
    ...
    print("\n".join([
        ...
        "",
        "# DT draft embedding: run on the HOST from /home/spott/DT, after all earlier jobs have ended.",
        "# Set draft_dir to the saved container-side draft directory. Preview first.",
        'bash backend/scripts/kantropos_upstream.sh embed-draft "${draft_dir:?Set the saved draft directory}"',
        'bash backend/scripts/kantropos_upstream.sh embed-draft "${draft_dir:?Set the saved draft directory}" --yes',
        "# Do not use the corpus-wide /embeddings endpoint for a DT draft.",
        "",
